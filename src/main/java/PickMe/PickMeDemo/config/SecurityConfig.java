    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests((requests) -> requests
                        .requestMatchers(HttpMethod.POST, "/login", "/register", "/mailConfirm", "/verifyCode").permitAll() // "/login", "/register"은 인증(로그인 여부)이 필요하지 않은 유일한 엔드포인트
                        .requestMatchers(HttpMethod.GET, "/nicknameDuplicate").permitAll() // "/nicknameDuplicate"도 회원 가입 시 중복 여부 확인하는 백엔드 처리이므로 인증이 필요하지 않음

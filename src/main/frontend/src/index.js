...
import { getSaveRecommendedList, getIsRecommededPortfolioView } from './hoc/request';
import { loginSuccess/*, lastVisitedEndpoint*/, setRecommendPortfolioView, saveRecommendedList } from './_actions/actions';
...
const savedRecommendList = getSaveRecommendedList();
const savedIsRecommendedPortfolioView = getIsRecommededPortfolioView();
...
if (storedAuthToken) { //저장된 토큰이 있다면 로그인완료 상태로 디스패치 -> f5누르면 index.js로 다시 오는데, 이거 때문에 로그인 상태가 유지되는 것임
    ...
    store.dispatch(setRecommendPortfolioView(savedIsRecommendedPortfolioView));
    store.dispatch(saveRecommendedList(savedRecommendList));
}
